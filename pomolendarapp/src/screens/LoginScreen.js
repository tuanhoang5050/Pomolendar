...
import { 
  ...
  KeyboardAvoidingView, Platform, Image, ScrollView, NativeModules
} from 'react-native';
...
export default function LoginScreen({ navigation }) {
  ...
  useEffect(() => {
    const forceStopDeepFocus = async () => {
      if (DeepFocus) {
        DeepFocus.stopDeepFocus();
      }
  ...
  const handleLogin = async () => {
    if (!email || !password) return Alert.alert("Error", "Please enter all information!");
        ...
        >
          <View className="items-center justify-center px-6 py-10">
            ...
            <View className="items-center mb-10">
              ...
              >
                <MaterialIcons name="timer" size={38} color="#ffffff" />
              </View>
            </View>
            ...
            >
              ...
              <View className="mb-2">
                <View className="flex-row items-center justify-between mb-2">
                  <Text className="text-[12px] font-bold text-[#5a413c] uppercase tracking-widest">Password</Text>
                  <TouchableOpacity onPress={() => navigation.navigate('ForgotPassword')}>
                    <Text className="text-[12px] font-bold text-[#c89d7d]">Forgot password?</Text>
                  </TouchableOpacity>
                </View>
                <View className="flex-row items-center bg-[#efeded] rounded-xl px-4">
              ...
              <View className="mt-6 flex-row justify-center">
                <Text className="text-[13px] text-[#8e706b]">Don't have an account? </Text>
                <TouchableOpacity onPress={() => navigation.navigate('Register')}>
                  <Text className="text-[13px] text-[#c89d7d] font-bold">Sign Up</Text>
                </TouchableOpacity>
              </View>
            </View>
